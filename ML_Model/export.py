...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info('TorchScript export failure: %s' % e)
    ...
    try:
        ...
        if bits < 32:
            if sys.platform.lower() == 'darwin':  # quantization only supported on macOS
                ...
                logger.info('quantization only supported on macOS, skipping...')

        f = opt.weights.replace('.pt', '.mlmodel')  # filename
        ct_model.save(f)
        logger.info('CoreML export success, saved as %s' % f)
    except Exception as e:
        logger.info('CoreML export failure: %s' % e)
    ...
    try:
        ...
        logger.info('TorchScript-Lite export success, saved as %s' % f)
    except Exception as e:
        logger.info('TorchScript-Lite export failure: %s' % e)
    ...
    try:
        ...
        logger.info('ONNX export failure: %s' % e)
